...
.post("/login", async (req, res) => {
  ...
  // Post for saving changes of the colors of events (does not work at the moment)
...
app.post('/updatenote', (req, res) => {
  ...
  Note.findByIdAndUpdate(data.id, { title: data.title },
    function (err, docs) {
      if (err) {
        console.log("err is 0" + err)
      }
...
//get request to retrieve all the todos from DB
...
//post request to add todo to the DB
...
//Delete request to remove the todo card
app.delete('/deletetodo', (req, res) => {
  //takes the data from the server (need ID)
  let data = req.body;
  //finds the card which has to be deleted by ID
...
app.post('/updatetodo', (req, res) => {
  ...
  let data = req.body;
  console.log(data)
  //finds the todo which has to be edited by ID and updates its title attribute
